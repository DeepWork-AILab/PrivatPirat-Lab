...
Это ограничение относится к публичному репозиторию и evidence. Для конкретного приватного операторского run владелец может явно разрешить использовать точные IP/SSH metadata и REALITY target/SNI в текущем приватном чате и готовой команде; порядок и границы такого разрешения задаёт `AGENTS.md`.
